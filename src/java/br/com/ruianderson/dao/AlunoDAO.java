...
public class AlunoDAO implements Dao_base<Aluno> {
    ...
    public int adicionar(Aluno aluno, Conexao con) {
        ...
        try {
            ...
            ps = (PreparedStatement) con.getPreparedStatement("INSERT INTO aluno (nome,celular,telefone,sexo,dt_nascimento,email) "
                    + "VALUES (?,?,?,?,?,?);");
    ...
    public int remover(Aluno aluno, Conexao con) {
        ...
        try {
            // Criando o comonado sql
            ps = (PreparedStatement) con.getPreparedStatement("DELETE FROM ALUNO WHERE ID = ?");
